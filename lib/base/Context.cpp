...
Context::Context(vkw::Device &device, vkw::DeviceAllocator &alloc,
                 const ContextCreateInfo &CI)
    : m_device(device), m_deviceAllocator(alloc) {
  // pre-initialize queue map
  for (auto &&index : m_device.get().physicalDevice().queueFamilies() |
                          std::views::transform(
...
Queue &Context::allocateQueue(const QueueCapsInfo &queueInfo) {
  auto &physDevice = m_device.get().physicalDevice();
...
void Context::freeQueue(Queue &queue) {
...
Queue &Context::m_allocateQueue(unsigned queueFamilyIndex,
                                unsigned queueIndex) {
  ...
  auto pQueue = new Queue{vkw::Queue(m_device, queueFamilyIndex, queueIndex),
                          /* sync */ false};
